...
//									{6,14,19,16,11,1,19,4,1,6,1,3,18,10,9,8,18,13,13,0}};
		...
		//Lista com a menor rota entre todas
		List<Integer> menorRotaTotal = new ArrayList<>();
		int menorDistanciaRotas = Integer.MAX_VALUE;
		
		//Para cada cidade ser a cidade inicial
		for (int cidadeInicial = 0; cidadeInicial < distanciaCidades.length; cidadeInicial++) {
			...
			cidadesJaVisitadas.add(cidadeInicial);
			
			//Declaração da lista de cidades não visitadas, com as cidades restantes
			List<Integer> cidadesNaoVisitadas = IntStream.rangeClosed(0, numeroDeCidades-1)
				    .boxed().collect(Collectors.toList());
			cidadesNaoVisitadas.remove(Integer.valueOf(cidadeInicial));
			
			List<Integer> listaCaminho = new ArrayList<Integer>();
			//adicionando na primeira e na última posição a cidade inicial
			listaCaminho.add(0, cidadeInicial);
			listaCaminho.add(1, cidadeInicial);
			...
			while(!cidadesNaoVisitadas.isEmpty()) {
				...
				//adicionar a cidade destino na lista de cidades visitadas e a remover das não visitadas
				cidadesJaVisitadas.add(cidadeDestinoMenorDist);
				cidadesNaoVisitadas.remove(Integer.valueOf(cidadeDestinoMenorDist));
				
				//salvar a lista com o menor caminho
...
//				System.out.println(listaCaminho + " Cidade adicionada: " + cidadeDestinoMenorDist);
			...
			int distanciaRotaFinal = calcularDistancia(listaCaminho.stream().mapToInt(Integer::intValue).toArray(), distanciaCidades);
			
			if (distanciaRotaFinal < menorDistanciaRotas) {
				menorDistanciaRotas = distanciaRotaFinal;
				menorRotaTotal = listaCaminho.stream().collect(Collectors.toList());
			}
			
			//imprimir distância e qual foi a cidade usada como inicial ao fim de cada iteração
//			System.out.println("Distância: " + distanciaRotaFinal + " (cidade inicial = " + cidadeInicial + ")");
		}
		
		System.out.println();
		System.out.println("Melhor rota: " + menorRotaTotal);
		System.out.println("Distância: " + menorDistanciaRotas);
